...
import { Zap, CheckCircle2, XCircle, Loader2, Filter, Play, Square, RotateCcw, ChevronDown, ChevronUp } from "lucide-react";
import { cn } from "@/lib/utils";

interface Subject { id: string; name: string; categoria: string; totalQuestoes: number; }

const BANCAS = [
  "CESPE/CEBRASPE","AOCP","FCC","VUNESP","ESAF","FGV","IDECAN","IBFC",
  "IADES","QUADRIX","NC-UFPR","FUNDATEC","FEPESE","OBJETIVA","NUCEPE",
];
...
export default function GerarMassaPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/materias/stats")
      ...
      .then((d: Record<string, { totalQuestoes: number; name?: string; categoria?: string }> | null) => {
        if (!d) return;
        const list: Subject[] = Object.entries(d).map(([id, v]) => ({
          id,
          name: v.name ?? id,
          categoria: v.categoria ?? "—",
          totalQuestoes: v.totalQuestoes ?? 0,
  ...
  async function start() {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        ...
        const d = await res.json() as { inserted?: number; error?: string; questoes?: unknown[] };
        if (!res.ok) throw new Error(d.error ?? "Erro");
        const inserted = d.inserted ?? d.questoes?.length ?? 0;
    ...
    <div className="p-6 max-w-5xl text-white space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Zap className="w-6 h-6 text-amber-400" /> Gerar Questões em Massa
        </h1>
        <p className="text-gray-500 text-sm mt-1">
          Selecione matérias, configure e clique em Gerar. O sistema chama a IA para cada matéria sequencialmente.
        </p>
      </div>
      ...
      <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs text-gray-400 mb-1">Banca</label>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Qtd por matéria</label>
            <input type="number" min={1} max={50} value={qtyPer} onChange={e => setQtyPer(Number(e.target.value))}
              className="w-full px-2 py-1.5 rounded-lg bg-white/5 border border-white/10 text-sm text-gray-200 focus:outline-none focus:border-indigo-500" />
          </div>
          <div className="sm:col-span-2 flex items-end gap-2">
            {!running ? (
              <button onClick={start} disabled={!selectedSubjects.length}
                className="flex-1 flex items-center justify-center gap-2 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-bold text-sm disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
        ...
        <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] overflow-hidden">
          <button onClick={() => setShowLog(v => !v)}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-500">{done}/{total} concluídos · {errs} erros</span>
            ...
            <div className="divide-y divide-white/[0.04] max-h-80 overflow-y-auto">
              {items.map((it, i) => (
                <div key={i} className={cn(
                  ...
                  it.status === "running" ? "bg-indigo-500/5" : ""
                )}>
                  ...
                  {it.status === "done" && (
                    <span className="text-xs text-emerald-400 font-mono">+{it.inserted} questões</span>
                  )}
                  {it.status === "error" && (
                    <span className="text-xs text-red-400 truncate max-w-48" title={it.error}>{it.error}</span>
                  )}
                </div>
              ))}
            </div>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2">
          ...
          <span className="px-3 py-1.5 text-xs text-gray-600 flex items-center">
            {selected.size} selecionadas · {subjects.reduce((s, sub) => s + sub.totalQuestoes, 0).toLocaleString()} questões no banco
          </span>
        </div>
              ...
              {filtered.map(s => (
                <label key={s.id}
                  className={cn(
                    ...
                    selected.has(s.id) ? "bg-indigo-500/[0.04]" : ""
                  )}>
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/[0.04] text-gray-500 border border-white/[0.06] flex-shrink-0">
                    {s.categoria}
                  </span>
                  <span className="text-xs text-gray-600 font-mono flex-shrink-0 w-16 text-right">
                    {s.totalQuestoes.toLocaleString()} q
                  </span>
                </label>
              ))}
              {filtered.length === 0 && (
                <p className="text-center py-6 text-gray-600 text-sm">Nenhuma matéria encontrada</p>
              )}
            </div>
          </div>
        )}
