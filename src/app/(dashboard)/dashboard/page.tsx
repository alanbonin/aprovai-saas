...
export default async function DashboardPage() {
  ...
  const [totalQuestions, totalSimulados, recentSimulados] = await Promise.all([
    prisma.progress.count({ where: { userId: dbUser.id } }),
    prisma.simuladoHistory.count({ where: { userId: dbUser.id } }),
    prisma.simuladoHistory.findMany({
  ...
  const correctCount = await prisma.progress.count({
    where: { userId: dbUser.id, correct: true },
  });
  ...
  const stats = [
    { label: "Questões respondidas", value: totalQuestions, icon: Target, color: "text-blue-400" },
    { label: "Taxa de acerto",       value: `${accuracy}%`, icon: TrendingUp, color: "text-green-400" },
    { label: "Simulados feitos",     value: totalSimulados, icon: Trophy,    color: "text-yellow-400" },
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <a href="/questoes" className="flex items-center gap-4 p-5 rounded-xl bg-indigo-600/10 border border-indigo-500/20 hover:bg-indigo-600/20 transition-colors">
            ...
            <div>
              <p className="font-medium">Praticar Questões</p>
              <p className="text-sm text-gray-400">Estude por matéria</p>
            </div>
          </a>
          <a href="/mentor" className="flex items-center gap-4 p-5 rounded-xl bg-purple-600/10 border border-purple-500/20 hover:bg-purple-600/20 transition-colors">
            ...
            <div>
              <p className="font-medium">Falar com Mentor</p>
              <p className="text-sm text-gray-400">IA especialista</p>
            </div>
          </a>
          <a href="/simulado" className="flex items-center gap-4 p-5 rounded-xl bg-yellow-600/10 border border-yellow-500/20 hover:bg-yellow-600/20 transition-colors">
            ...
            <div>
              <p className="font-medium">Fazer Simulado</p>
              <p className="text-sm text-gray-400">Teste seus conhecimentos</p>
            </div>
          </a>
        </div>
      </div>
                  ...
                  <tr key={s.id} className="hover:bg-white/5">
                    ...
                    <td className="px-4 py-3">
                      <span className={`font-medium ${
                        (s.correct / s.total) >= 0.7 ? "text-green-400" : "text-red-400"
                      }`}>
                        {Math.round((s.correct / s.total) * 100)}%
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-400">
                      {Math.floor(s.timeSecs / 60)}min
                    </td>
                  </tr>
                ))}
              </tbody>
